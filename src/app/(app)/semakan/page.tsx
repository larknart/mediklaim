...
export default async function SemulaPage() {
  ...
  const [reviewClaims, paidPendingClaims] = await Promise.all([
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          {reviewClaims.length} menunggu semakan · {paidPendingClaims.length} menunggu pembayaran
        </p>
      </div>
      ...
      {/* Queue 2: Approved, needs payment recording */}
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-semibold text-gray-700 flex items-center gap-2">
            <Banknote className="w-4 h-4" />
            Menunggu Pembayaran ({paidPendingClaims.length})
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {paidPendingClaims.length === 0 ? (
            <div className="py-8 text-center text-gray-400 text-sm">Tiada tuntutan menunggu pembayaran.</div>
          ) : (
            <div className="divide-y">
              {paidPendingClaims.map((claim) => (
                <Link
                  key={claim.id}
                  href={`/tuntutan/${claim.id}`}
                  className="flex items-center gap-4 p-4 hover:bg-gray-50"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      ...
                      RM {Number(claim.totalApprovedMyr ?? claim.totalClaimedMyr).toFixed(2)} diluluskan
                    </p>
                  </div>
                  <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100">Diluluskan</Badge>
                </Link>
              ))}
            </div>
          )}
